    ...
    <div x-data="{ open: @entangle('isOpen') }" 
        ...
        <div class="fixed inset-0 z-50 overflow-y-auto p-4 sm:p-6 md:p-20">
            <div x-on:click.away="open = false" 
                ...
                @if($results && count($results->hits) > 0)
                    <ul class="max-h-72 scroll-py-2 overflow-y-auto py-2 text-sm text-gray-800">
                        @foreach($results->hits as $hit)
                            @php
                                // Match the raw search hit to its hydration model
                                $product = $products[$hit->document['id']] ?? null;
                            @endphp

                            @if($product)
                                <li class="group select-none px-4 py-2 hover:bg-gray-50 transition">
                                    {{-- Use the official catch-all slug architecture --}}
                                    <a href="/{{ $product->defaultUrl?->slug }}" class="flex items-center space-x-4" wire:navigate>
                                        <div class="h-10 w-10 shrink-0 overflow-hidden rounded bg-gray-100">
                                            @if($product->getFirstMediaUrl('images', 'medium'))
                                                <img src="{{ $product->getFirstMediaUrl('images', 'medium') }}" alt="" class="h-full w-full object-cover" />
                                            @else
                ...
                @elseif($results)
                    <div class="px-4 py-6 text-center text-sm sm:px-14">
